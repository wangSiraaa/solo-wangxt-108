...
负责人可在**任意双批次比较页**创建结论备注（`comparison_notes` 表），每条备注创建时冻结：

- 批次对（A/B，无方向匹配）、**对齐模式**（physical/phase）、平滑窗与 RoR 窗参数；
- 两批**当时的有效事件版本快照**（`event_version_a/b`）与锚点/可比性摘要（`anchor_snapshot`）；
  之后事件再被人工修正，**旧备注的依据不变**。
- 状态 `followup 待跟进 / confirmed 已确认 / discarded 已废弃`、责任人、截止日期。

每次状态变更写一行审计（`note_status_revisions`：旧状态、新状态、原因、操作人、时间），
重复置为同一状态返回 409。接口：

- `POST /api/notes`（创建并快照）、`GET /api/notes?batch_id=&other_id=&status=`（列表/过滤）；
- `PUT /api/notes/{id}/status`（带原因的状态流转）、`GET /api/notes/{id}/revisions`（轨迹）。
- 批次详情 `GET /api/batches/{id}` 与比较结果 `/api/compare` 都内联关联备注。

前端 `NotesPanel.svelte`：比较页提供创建表单（明示“将绑定批次/对齐/参数/事件版本快照”）、
**全部/待跟进/已确认/已废弃筛选**、历史列表、状态按钮与“快照/轨迹”展开；
**切换对齐方式只显示同模式备注，切换批次对只显示该对备注**；批次详情页只读列出该批次的全部备注。
播种两条合成样例：相位对齐（待跟进，zhang，带截止）与物理对齐（已确认，wang）。

第三阶段浏览器验证 `browser_check3.py`（22 项）+ `test_notes_api.py`（7 项）覆盖：
创建即快照（五事件/锚点）、事件后续修正后旧备注快照仍为旧值、状态三段流转可追溯、
按状态/批次过滤、前端切对齐方式与批次对后只显示对应备注、批次详情页关联展示。

## 阶段复盘标签（第四阶段）

负责人在批次详情为**回温/变黄/一爆/出豆等已确认事件**打一个或多个标签并附简短说明
（如“回温偏慢”“一爆判断偏晚”“尾段火力过强”）。`event_tags` 表在创建时冻结
**所绑定的事件版本**：`bound_event_id / event_t_s / event_source`，并记录创建人、创建时间。

- 事件后续被人工修正时，**旧标签的快照不变**；列表服务比对当前有效事件，
  给每条标签返回 `changed` 与 `current_state`（current/changed/event_missing）、
  `current_event`（当前 id/时间/来源）。前端对变更标签显示红色
  “⚠ 当前事件已变更”，并**同时呈现标签创建时的原时间（删除线）与当前时间**，未变更的显示
  “✓ 与当前事件一致”，避免把旧复盘结论误读为针对当前事件。
- 删除标签（`DELETE /api/batches/{id}/tags/{tid}`）只删标签本身，
  **不触碰 events 与 event_revisions**（接口返回 `events_unchanged`，并有测试断言）。
- 接口：`POST /api/batches/{id}/tags`、`GET /api/batches/{id}/tags?event_kind=`、
  `DELETE ...`；批次详情 `tags` 字段与导出包 `event_tags` 都包含标签。
  标签不进入对齐、RoR、阶段指标，双批次比较结构不受影响。
- 前端 `TagsPanel.svelte`：创建入口（选事件+标签+说明）、全部/回温/变黄/一爆/出豆筛选、
  标签列表、删除（二次确认）；缺失已确认事件的批次（如批次 3 缺一爆）无法为该事件建标签（409）。
- 播种三条样例：一爆“一爆判断偏晚”、出豆“尾段火力过强”、变黄“回黄正常”。

第四阶段浏览器验证 `browser_check4.py`（23 项）+ `test_tags_api.py` 覆盖：
事件快照保存、修正事件后旧标签快照不变且页面同时显示原/当前时间、按事件筛选、
删除后页面与接口同步且事件/修订不变、事件编辑/批次导出（含标签且指标可复现）/双批次比较均不受影响。

### 标签处理闭环（待处理/已采纳/已忽略）

每条标签有处理状态 `open 待处理 / adopted 已采纳 / ignored 已忽略`，负责人可填写处理结论
（`resolution`）与处理人。状态流转写入只追加表 `tag_status_revisions`
（旧状态、新状态、结论、处理人、时间），**不改写标签创建时绑定的事件快照**，
也不触碰 `events` 与 `event_revisions`；重复置为同一状态返回 409。

- 接口：`PUT /api/batches/{id}/tags/{tid}/status`（带结论的状态更新）、
  `GET /api/batches/{id}/tags/{tid}/revisions`（处理历史）；
  `GET .../tags?event_kind=&status=` 支持事件、处理状态两种过滤（可组合）。
- 批次详情的每个标签带当前 `status/resolution/resolved_by/resolved_at`、
  `last_decision`（最近一次真正处理）与完整 `status_history`；导出包 `event_tags` 同样含全部这些字段。
- 前端 `TagsPanel`：状态徽标（颜色区分三态）、卡片上“待处理/已采纳/已忽略”切换（prompt 填结论）、
  事件+状态双行筛选、“最近处理”一行与“完整历史”展开（时间/状态变化/结论/处理人）。
- 删除标签级联删除其处理历史（外键 ON DELETE CASCADE），事件、事件修订、批次比较、既有备注均不受影响
  （接口返回 `tag_history_deleted`，并有断言）。
- 播种样例覆盖三态：一爆“一爆判断偏晚”=待处理、出豆“尾段火力过强”=已采纳（含下批动作）、
  变黄“回黄正常”=已忽略（经验性备注）。

第五阶段浏览器验证 `browser_check5.py`（31 项）覆盖：三态徽标与最近处理信息、
按状态及事件+状态组合筛选、待处理→已采纳→已忽略的历史可追溯、事件后续修正不改变标签快照与既有处理历史、
导出含处理状态/结论/历史、删除级联处理历史但事件/修订/比较/备注不变。
